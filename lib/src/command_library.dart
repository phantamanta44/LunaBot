...
void register(Commander registry) {
  registry.register(new Command('echo')
    ..withDescription('Prints the provided arguments.')
    ..withExecutor((List<String> args, Message ctx, LunaBot bot,
        List<String> stdin) {
  ...
  registry.register(new Command('grep')
    ...
    ..withExecutor((List<String> args, Message ctx, LunaBot bot, List<String> stdin) {
      ...
      if (parsed['ignore-case'])
        stdin = stdin.map((s) => s.toLowerCase());
